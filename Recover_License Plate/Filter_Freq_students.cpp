...
float H_re[MaxBMPSizeX][MaxBMPSizeY]; 
float H_im[MaxBMPSizeX][MaxBMPSizeY];
float tmp_re[MaxBMPSizeX][MaxBMPSizeY]; 
float tmp_im[MaxBMPSizeX][MaxBMPSizeY];
...
int main(int argc, char *argv[])
{
	...
	open_bmp("CarPlate2_blur2.bmp", R, R, R, width, height); // for gray images
//	open_bmp("cameraman.bmp", R, R, R, width, height); // for gray images
  ...
  int T=1;
   for (i = 0; i < height; i++){
       for (j = 0; j < width; j++){
       	float a=0.14;
          float c=PI*(i-128)*a;
          H_re[i][j]=(1/(c))*sin(c)*cos(-c);
          H_im[i][j]=(1/(c))*sin(c)*sin(-c);
         // printf("%f %f\n",H_re[i][j],H_im[i][j]);
         
        	tmp_re[i][j]  =sp_re[i][j] ;
          	tmp_im[i][j]  =sp_im[i][j] ;
       }
    }
  ...
  for (i = 0; i < height; i++){
       for (j = 0; j < width; j++){
       	float H;
  		//	if((H_re[i][j]!=0 && H_im[i][j]!=0))
  		
		  {
		  	H=hypot(H_re[i][j],H_im[i][j]);
			if(H>=0.01)
		  	{
		  		//sp_re[i][j] =sp_re[i][j]*H_re[i][j];
         //sp_im[i][j]=sp_im[i][j]*H_im[i][j] ;
		  		sp_re[i][j] = ((tmp_re[i][j]* H_re[i][j])+(tmp_im[i][j]* H_im[i][j]))/(pow(H_re[i][j],2)+pow(H_im[i][j],2));
            	sp_im[i][j] = ((tmp_im[i][j]* H_re[i][j])-(tmp_re[i][j]* H_im[i][j]))/(pow(H_re[i][j],2)+pow(H_im[i][j],2));
            
        //   	printf(" %f\n",sp_re[i][j]);
            	
            	//	sp_re[i][j]= H_re[i][j]*tmp_re[i][j]-H_im[i][j]*tmp_im[i][j] ;
		  	//	sp_im[i][j]=H_re[i][j]*tmp_im[i][j] +H_im[i][j]*tmp_re[i][j];
		  	}
   ...
   spectrum_shift(mm);
	...
	save_bmp("CarPlate2_blur2_new.bmp", R, R, R); // for gray images
	printf("Job Finished!\n");
